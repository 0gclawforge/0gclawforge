...
async function downloadRealm(rootHash: string, _tokenId: string, chainId: number): Promise<any> {
  try {
    return await downloadJSON(rootHash, getStorageConfig(chainId));
  } catch (err) {
    if (chainId !== 16602) {
      return await downloadJSON(rootHash, getStorageConfig(16602));
    }
    throw err;
  }
}
