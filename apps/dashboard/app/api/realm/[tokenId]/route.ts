...
async function downloadRealmRecord(rootHash: string, _tokenId: string, chainId: number) {
  try {
    return await downloadJSON<any>(rootHash, getStorageConfig(chainId, false));
  } catch (err) {
    // Realm may have been uploaded on a different network's storage indexer.
    if (chainId !== 16602) {
      return await downloadJSON<any>(rootHash, getStorageConfig(16602, false));
    }
    throw err;
  }
}
